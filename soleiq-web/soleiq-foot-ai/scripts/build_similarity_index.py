#!/usr/bin/env python3
"""Embed the reference bank and build the FAISS index."""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPO_ROOT))

from src.config import load_config, project_root  # noqa: E402
from src.similarity.index import build_index  # noqa: E402


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--config", default="config.yaml")
    args = ap.parse_args()
    cfg = load_config(args.config)

    artifacts = project_root() / cfg["paths"]["artifacts"]
    ckpt_path = artifacts / "best.ckpt"
    if not ckpt_path.exists():
        raise SystemExit(f"checkpoint not found at {ckpt_path}; run `make train` first")

    info = build_index(cfg, ckpt_path)
    print("[similarity] index built:", info)


if __name__ == "__main__":
    main()
