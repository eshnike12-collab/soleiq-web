...
export default function LegacyAdminPage() {
  ...
  useEffect(() => {
    ...
    const membership = auth.memberships.find(
      (row) => row.status === "active" && row.role === "admin"
    );
    router.replace(
      membership?.organizations?.slug
        ? `/h/${membership.organizations.slug}/admin`
        : "/home"
    );
  ...
  return <div className="flex min-h-screen items-center justify-center text-[15px] text-ink-soft">Resolving hospital…</div>;
}
